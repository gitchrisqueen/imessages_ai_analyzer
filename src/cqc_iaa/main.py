...
class EnvironmentLoader:
    ...
    def load_env():
        ...
        return {
            ...
            'MAIN_COLOR_HEX': os.getenv('MAIN_COLOR_HEX'),
            'SECONDARY_COLOR_HEX': os.getenv('SECONDARY_COLOR_HEX'),
        }
...
class DataProcessor:
    ...
    @staticmethod
    def filter_messages_by_date_only(messages, days=7):
        test_data = {}
...
class PDFGenerator(FPDF):
    def __init__(self, main_color_hex='000000', secondary_color_hex='B8860B'):
        super().__init__()
        self.main_color_hex = main_color_hex
    ...
    def get_brand_colors_hex(self, level: int = 0):
        colors = {0: self.main_color_hex, 1: self.secondary_color_hex}
    ...
    def header(self):
        ...
        self.cell(0, 10, 'iMessages Political Analysis', 0, 1, 'C')
...
def main():
    ...
    # test_data_ids = DataProcessor.filter_messages_by_date_only(messages)
    # Display all the unique ids
    # print(test_data_ids.keys())

    test_data = DataProcessor.filter_messages(messages, env_vars['FOCUS_GROUP'])

    summarizer = Summarizer(api_key=env_vars["OPENAI_API_KEY"])

    user_summaries = OrderedDict()
    for user_id in env_vars['FOCUS_GROUP']:  # Loop in order of the focus group list
        if user_id in test_data:
            messages = test_data[user_id]
            messages.reverse()
            text = "\n".join(messages)
